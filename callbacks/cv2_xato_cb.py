...
from services.cv2_fsm import anketa2_fsm # FSM o'zgardi[cite: 8]
from keyboards.cv2_kb import anketa2_lang_keyboard # Keyboard o'zgardi[cite: 8]
...
async def cv2_proceed_callback(client, callback):
    ...
    if balance >= CV_PRICE:
        anketa2_fsm.finish(user_id) # FSM o'zgardi[cite: 8]
        anketa2_fsm.update_data(user_id, "cv_lang", lang) # FSM o'zgardi[cite: 8]
        anketa2_fsm.update_data(user_id, "waiting_for_format", False) # FSM o'zgardi[cite: 8]
        ...
        # Til tugmasi keyboard'i o'zgardi[cite: 8]
